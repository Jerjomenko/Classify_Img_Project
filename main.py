...
ScreenManager:
    MDScreen:
        ...
        MDLabel:
            ...
            y_hint: .5
            x_hint: 2.0
            pos_hint: {"center_x": self.x_hint, "center_y": self.y_hint}
            font_style: "H6"
...
class ImageClassify(MDApp):
    ...
    def anime(self, label, *args):
        anim = Animation(x_hint=.7, y_hint=.75,  duration=1.7)
        anim.start(label)
    ...
    def recognition(self, pathImage, model):
        ...
        percentCImg = round(listProbablyImg[0, predImg] * 100, 2)
        nameClass = numClass[predImg]
        # выводим информацию по распознованию в лейбл
        pred = (f"This image belongs probability to class {numClass[predImg]}")
